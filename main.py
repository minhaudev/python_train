...
# 8. Nhận dạng bằng Transformer
def identify_with_transformer(filename: str, model_name: str = "facebook/wav2vec2-large-xlsr-53"):
    print("Loading Transformer model...")
    processor = AutoProcessor.from_pretrained(model_name)
    model = AutoModelForAudioClassification.from_pretrained(model_name)

    print("Processing audio...")
    fs, audio = read_audio_file(filename)
    audio = torch.tensor(audio, dtype=torch.float32).unsqueeze(0)  # Add batch dimension
    ...
    print(f"Predicted class by Transformer: {predicted_class}")
...
if __name__ == "__main__":
    ...
    # Nhận dạng bài hát với dấu vân tay
    identify_song("output.wav", db_filename="database.pickle", song_index_filename="song_index.pickle")
    # Nhận dạng bài hát với Transformer
    identify_with_transformer("output.wav")
